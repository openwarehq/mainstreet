 ...
 *
 * One file, inline CSS, no build step, no framework, no external requests. A
 * local business site does not need 200KB of JavaScript to show an address and
 * a phone number, and a single file is something you can hand over, host
 * anywhere, or attach to an email — which matters when the whole point is to
 * put it in front of an owner who has not asked for it yet.
 *
 * The hero artwork is generated SVG rather than a stock photograph. Two
 * reasons, and the second is the important one: stock photography of a business
 * that is not theirs is a small lie, and nobody has photographs of these
 * premises anyway.
 */
...
function renderSection(s: Section, spec: SiteSpec): string {
  ...
  <div class="art">${heroArt(spec)}</div>
  ...
  <div class="split">
    ...
    <div class="pin">
      ...
      <div><strong>${esc(spec.business)}</strong><br><span style="color:var(--muted);font-size:14.5px">${s.lat.toFixed(4)}, ${s.lon.toFixed(4)}</span></div>
    </div>
  </div>
</div></section>`;
